...
export const Inventory = () => {
	...
	let filter = product?.filter((el) => {
		if (el?.stock[0]?.quantity > gte && el?.stock[0]?.quantity <= lte) {
			return el;
		}
	});
	...
	return (
		<Box bgColor={'#FFFFFF'} h={'100%'} borderRadius={'1.5em'} p={'1.5em'}>
			<Flex bg={'#FFFFFF'} h={'100%'} borderRadius={'1em'} padding={''} flexDir={'column'}>
				<Flex w={'100%'}>
					<Flex w={'27em'}>
						<Link to={''}>
							<Button
							...
							{filter?.map((el) => {
								return (
									<Tr>
										...
										<Td textAlign={'center'}>
											{el?.stock[0]?.quantity === 0 || el?.stock[0]?.quantity === 'false' ? (
