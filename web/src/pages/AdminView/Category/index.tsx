...
export const Category = () => {
	...
	return (
		<Box w={'100%'} h={'100%'} bgColor={'#FFFFFF'} borderRadius={'1.5em'} p={'1.5em'}>
			<Flex flexDir={'column'}>
						...
						>
							...
							<Tbody position={'relative'} top={'-.5em'} color={'#6D6D6D'} fontWeight={'500'}>
								{category?.map((el: any, index: number) => {
									return (
										<Tr w={'10px'} cursor={'pointer'} key={index} p={'.875em'} bgColor={'#FAFAFA'}>
											<Td textAlign={'center'}>{el?.id}</Td>
											<Td textAlign={'center'}>{el?.product_category_name}</Td>
											<Td textAlign={'center'}>
