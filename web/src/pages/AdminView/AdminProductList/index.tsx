...
export const AdminProductList = () => {
	...
	const fetchCategory = async (): Promise<any> => {
		try {
			const res = await axios.get(`${ROUTE}/category/category`);
			await fetchProduct();
	...
	const fetchGroup = async () => {
		try {
			const res = await axios.get(`${ROUTE}/category/group`);
			await fetchProduct();
	...
	return (
		<Flex h={'100%'} borderRadius={'1em'} padding={''} flexDir={'column'}>
			<Box alignSelf={'flex-end'}>
				<CreateProductModal status={status} group={group} category={category} />
			</Box>
			<Spacer m={'.3em 0'} />
						...
						<InputGroup w={'11em'}>
							...
							<Input
								...
								onKeyDown={(e: any) => {
									if (e.keyCode == 13) {
										// e.preventDefault();
										setSearchName(String((e.target as HTMLInputElement).value));
									...
									{product.map((el, index) => {
										return (
											<Tr cursor={'pointer'} key={index} p={'.875em'} bgColor={'#FAFAFA'}>
												...
												<Td textAlign={'center'}>{el?.product_category?.product_category_name}</Td>
												<Td textAlign={'center'}>{el?.product_group?.product_group_name}</Td>
