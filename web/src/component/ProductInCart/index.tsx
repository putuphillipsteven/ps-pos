...
export default function ProductInCart(props: any) {
	...
	const [totalPPrice, setTotalPPrice] = useState(props.product_price * props.qty);
	useEffect(() => {
	...
	return (
		<HStack boxShadow={'sm'} borderRadius={'0.5em'}>
			<Box>
				<Image
					src={`${import.meta.env.VITE_APP_API_IMAGE_URL}/product/${props?.image}`}
					borderRadius={'0.5em'}
					boxSize={'5em'}
				/>
			</Box>
			<VStack align={'stretch'} spacing={'0'}>
				...
				<Text fontWeight={'400'}>{`${props.qty} X ${toRupiah(props.product_price)}`}</Text>
				<Text display={props?.display}>Sum : {toRupiah(totalPPrice)}</Text>
			</VStack>
			<Spacer />
