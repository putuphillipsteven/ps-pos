...
export const ProductList = (props: any) => {
	...
	useEffect(() => {
		fetchProduct();
	}, [page, pageSize, sortOrder, sortField, setGroupId, groupId, setCatId, catId]);
	...
	return (
		<Box bgColor={'#FFFFFF'} h={'100%'} borderRadius={'1.5em'} p={'1.5em'} boxShadow={'inner'}>
			<Flex flexDir={'column'} h={'100%'}>
				...
				>
					{product
						.filter((el) => {
							if (search === '') {
								return el;
							} else if (el.product_name.toLowerCase().includes(search.toLowerCase())) {
								return el;
							}
						})
						.map((el, index) => (
				...
				<Spacer />
				<Box overflow={'hidden'} bgColor={'#FAFAFA'} p={'.875em'} borderRadius={'1em'}>
