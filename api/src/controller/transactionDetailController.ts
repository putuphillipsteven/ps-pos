...
export const createTransactionDetailController = async (req: Request, res: Response) => {
	try {
		const { transaction_id, product_id, qty, total_price } = req.body;
...
export const getAllTransactionDetailController = async (req: Request, res: Response) => {
	try {
...
export const getTransactionDetailQueryByTransactionIdController = async (
	...
	try {
		...
		const result = await getTransactionDetailQueryByTransactionIdService(Number(transaction_id));
		return res.status(200).json({
			message: 'success',
			data: result,
		});
